...
int EASY_PERCENTAGE = 75;
int MEDIUM_PERCENTAGE = 88;
int HARD_PERCENTAGE = 97;
...
void userTurn() {//still have to rotate
    ...
    while (gotGoodInput == false) {
        ...
        if (counter == 2) {
            if (gameBoard[boardRow][col - 1] == EMPTY) {
                ...
                gotGoodInput = true;
                addTurnToBoard(boardRow, col - 1, USER_PIECE);
                break;
            } else {
    ...
    if (mustRoate() ) {
        ...
        while (gotGoodInput == false) {
            printf("Enter board section to rotate (1 for top left, 2 for top right, 3 for bottom right, 4 for bottom left): ");
            scanf("%d", &section);
...
//    } else {
    ...
    index = 0;
    bool keepGoing = true;
    while (availableSpots[index][0] != -1 && keepGoing == true) {
        a = availableSpots[index][0];
        b = availableSpots[index][1];
    ...
    if (foundWinningMove == false) { ///===============================defensive move============================================
        ...
        ///=============================================  strategy =================================================================
        if (foundDefensiveMove == false) {//strategic move
...
int main (void) {
    initialize();
    bool gotGoodInput = false;
    int turnToStart, winner, difficulty;
